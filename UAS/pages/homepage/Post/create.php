  ...
  <div class="container-fluid bd-layout p-0">
    ...
    <main class="bd-main p-3 bg-light">
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Handle file upload
        $target_dir = "../../article/uploads/";
        $target_file = $target_dir . basename($_FILES["file"]["name"]);
        $target_db = "uploads/" . basename($_FILES["file"]["name"]);
        move_uploaded_file($_FILES["file"]["tmp_name"], $target_file);

        // Call createArticle function
        $image = $target_db;

        $result = createArticle($user_id, $title, $content, $image, $category_id);
